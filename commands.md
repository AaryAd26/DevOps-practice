1. This is the command folder where the commands will be saved so that we cam use the commands whenever and wherever needed so that it can be easily understandable.

2. This is Helpful because it is easily availabel and accessable.

3. The main advantage of this file is that it stores all the information regarding the commands that we gonna be using throughout.

4. this line in this file is for commiting the md file to master branch.

5. This is the 5th commit of the file.

6. working in this file on different branch so that i can learn git stash and git stash pop 
 
